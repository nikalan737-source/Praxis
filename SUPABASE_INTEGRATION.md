...
- **`lib/supabase/server.ts`** – Server client with cookie handling for API routes

### Auth
- **`contexts/AuthContext.tsx`** – Auth state, `signInWithOtp`, `signOut`
- **`app/auth/callback/route.ts`** – Magic link callback
- **`components/LoginModal.tsx`** – Email magic link sign-in modal

### Nav
- **`components/NavClient.tsx`** – Login/Sign out UI in nav; "Sign in" opens modal, "Sign out" logs out

### Profile Protection
- **`app/profile/page.tsx`** – Redirects to `/community` if not logged in

### API Routes (DB-backed)
- `GET /api/theories` – List theory blocks
- `GET /api/theories/[id]` – Single theory
- `POST /api/publish` – Publish generated theory to community (auth required)
- `POST /api/saves` – Toggle save (auth required)
- `GET /api/save-counts?ids=` – Save counts per theory
- `GET /api/user-saves` – User's saved theory IDs
- `POST /api/upvote` – Toggle upvote (auth required)
- `GET /api/upvote-counts?ids=` – Upvote counts
- `GET /api/user-upvotes` – User's upvoted IDs
- `POST /api/experiment-logs` – Create log (auth required)
- `GET /api/experiment-logs?theoryId=` – List logs
- `DELETE /api/experiment-logs/[id]` – Delete log
- `GET /api/experiment-log-stats?ids=` – Log count + avg outcome per theory

### Hooks
- **`hooks/useSavedTheories.ts`** – Fetches from `/api/user-saves`, toggles via `/api/saves`
- **`hooks/useUpvotes.ts`** – Fetches counts + user upvotes, toggles via `/api/upvote`
- **`hooks/useTheoryCounts.ts`** – Fetches save/upvote/log counts from APIs
- **`hooks/useExperimentLogs.ts`** – Fetches from `/api/experiment-logs`, create/delete via API

### Layout
- **`app/layout.tsx`** – Wraps app with `AuthProvider`, uses `NavClient`

### Publish to Community Flow
- **`app/generate/page.tsx`** – Generate returns theory locally (no DB insert). "Publish to Community" button:
  - If logged in → POST `/api/publish` → theory saved with `created_by` = user id
  - If not logged in → store theory in sessionStorage, open LoginModal with `redirectAfterLogin="/generate?publish=1"`
  - After magic-link auth, user returns to `/generate?publish=1` → theory restored from sessionStorage → Publish button shown
- **`app/api/publish/route.ts`** – Requires auth; inserts into `theory_blocks` with `created_by`
...
create policy "theory_blocks_insert" on public.theory_blocks for insert with check (auth.uid() is not null and auth.uid() = created_by);
...
2. **Authentication → Providers → Email**: Enable Email provider for magic link sign-in.
